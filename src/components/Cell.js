...
const Cell = (props) => {
    ...
    const MainView = styled.div`
        ...
        width: 100%;
        ${({isTileMode}) => isTileMode ? `height: 150px` : `height: 362px` };
        box-sizing: border-box;
        ${({isTileMode}) => isTileMode && `margin: 5px 5px` };
        ${({isTileMode}) => isTileMode ? `padding: 5px 8px` : `padding: 20px 25px` };
        ${({inverted}) => inverted ? `background-color: #000000` : `background-color: #979797` };
        @media ${device.tablet} {
            ${({isTileMode}) => isTileMode ? `height: 250px` : `min-height: 310px` };
            ${({isTileMode}) => isTileMode ? `padding: 15px 18px` : `padding: 30px 38px` };
        }
        @media ${device.laptop} {
            ${({isTileMode}) => isTileMode ? `height: 250px` : `min-height: 440px` };
            ${({isTileMode}) => isTileMode ? `padding: 15px 18px` : `padding: 40px 50px` };
        }
        @media ${device.laptopL} {
            ${({isTileMode}) => isTileMode ? `height: 150px` : `min-height: 440px` };
            ${({isTileMode}) => isTileMode ? `padding: 15px 18px` : `padding: 40px 50px` };
        }
    `;
    const Title = styled.div`
        ...
        ${({isTileMode}) => (isTileMode ? `font-size: 18px` : `font-size: 25px;`) };
    ...
    const Body = styled.div`
        ...
        ${({isTileMode}) => isTileMode ? `font-size: 8px` : `font-size: 14px` };
    ...
    const Links = styled.div`
        width: 100%;
        font-family: "TheinhardtPan-Light";
        ${({isTileMode}) => isTileMode ? `font-size: 8px` : `font-size: 12px` }
        ${({inverted}) => inverted ? `color: #ffffff` : `color: #333333` }
        @media ${device.tablet} {
